...
def _role_context(lead) -> str:
    """Turn noisy source titles into natural, conservative outreach language."""
    title = (lead.title or "").lower()
    if any(word in title for word in ("ciso", "security", "cyber", "risk", "grc", "dpo")):
        return "your responsibility for security"
    if any(word in title for word in ("cio", "it", "infrastructure", "digital transformation")):
        return "your technology and IT leadership remit"
    if any(word in title for word in ("cto", "engineering", "technology")):
...
def draft_for(lead) -> GeneratedContent:
    ...
    linkedin = LinkedInSequence(
        ...
        messages=[
            LinkedInMessage(step=1, delay_days=2, kind="follow_up", body=f"Thanks for connecting, {first}. Given {role_context} at {company}, is {theme} on the agenda?"),
    ...
    return GeneratedContent(emails=emails, linkedin=linkedin, whatsapp=whatsapp, style_notes="Role/company-only draft. No unverified trigger, timing claim, or external fact. Individual research is required before send approval.")
